...
//						idLength = 7;
				...
				// サービス指定コマンド
//				BYTE selectCommand[] = {0xFF, 0xA4, 0x00, 0x01, 0x02, 0x88, 0xB4}; 
	
				// カードにコマンド送信してデータを受信する
//				RC = SCardTransmit( sCardHandle, SCARD_PCI_T1, selectCommand, sizeof(selectCommand), NULL, recieveBuffer, &recieveBufferSize );
//					if(RC != SCARD_S_SUCCESS){onSCardError( jsonProperty, "TransmitCommand", sCardContext, sCardHandle);	return 1;}

				BYTE selectCommand[] = {0xFF, 0xA4, 0x00, 0x01, 0x02, 0x0B, 0x00}; 

					// カードにコマンド送信してデータを受信する
				RC = SCardTransmit( sCardHandle, SCARD_PCI_T1, selectCommand, sizeof(selectCommand), NULL, recieveBuffer, &recieveBufferSize );
					if(RC != SCARD_S_SUCCESS){onSCardError( jsonProperty, "TransmitCommand", sCardContext, sCardHandle);	return 1;}

				BYTE recieveReadBuffer[16 + 2];	
				DWORD recieveReadBufferSize = sizeof(recieveReadBuffer);
					
					
				// readコマンド
				BYTE readCommand[] = {0xFF, 0xB0, 0x00, 0x82, 0x00, 0x00}; 

				// カードにコマンド送信してデータを受信する
				RC = SCardTransmit( sCardHandle, SCARD_PCI_T1, readCommand, sizeof(readCommand), NULL, recieveReadBuffer, &recieveReadBufferSize );
				if(RC != SCARD_S_SUCCESS){onSCardError( jsonProperty, "TransmitCommand", sCardContext, sCardHandle);	return 1;}
				...
				for(int i = 0; i < idLength; i++){
					...
					idstream << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(recieveReadBuffer[i]);
